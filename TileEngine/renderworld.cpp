						...
						while (pNode != NULL)
						{
							...
							if (fRenderTile && !fHiddenTile)
							{
								...
								else if (uiLevelNodeFlags & LEVELNODE_DISPLAY_AP && !(uiFlags&TILES_DIRTY))
								{
									...
									INT16 remainingAp( gusSelectedSoldier->bActionPoints - static_cast<INT16>(pNode->uiAPCost) );
									mprintf_buffer(pDestBuf, uiDestPitchBYTES, TINYFONT1, sX, sY, L"%d", remainingAp);
