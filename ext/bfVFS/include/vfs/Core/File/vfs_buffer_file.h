...
namespace vfs
{
	class VFS_API BufferFile : public TFileTemplate<IReadable,IWritable>
	{
		typedef TFileTemplate<IReadable,IWritable> BaseClass_t;

	public :
		VFS_SMARTPOINTER(BufferFile);
		...
		// convenience method
		void                    copyToBuffer(ReadableFile_t* file);

	protected:
		BufferFile();
		BufferFile(Path const& filename);
