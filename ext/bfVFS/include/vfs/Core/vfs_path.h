...
namespace vfs
{
	...
	public:
		...
		const String::char_t*   c_str     () const;
		const String::str_t&    c_wcs     () const;
		const String::str_t&    operator()() const;

		std::string             to_string () const;

		Path&                   operator+=(Path   const& p);
		Path&                   operator+=(String const& p);
		...
		void                    splitLast (Path &head,  Path &last) const;
		void                    splitFirst(Path &first, Path &tail) const;

		bool                    extension (String &ext) const;

		bool                    operator==(Path const& p2);
