...
vfs::BufferFile::BufferFile()
: BaseClass_t(vfs::Path()), m_isOpen_read(false), m_isOpen_write(false)
{
	m_buffer.str("");
}

vfs::BufferFile::BufferFile(vfs::Path const& fileName)
: BaseClass_t(fileName), m_isOpen_read(false), m_isOpen_write(false)
{
	m_buffer.str("");
}
...
vfs::size_t vfs::BufferFile::getSize()
{
	IS_FILE_VALID();
...
bool vfs::BufferFile::isOpenRead()
{
	return m_isOpen_read;
}

bool vfs::BufferFile::openRead()
{
	if(!m_buffer.good())
	{
		return false;
...
vfs::size_t vfs::BufferFile::read(vfs::Byte* data, vfs::size_t bytesToRead)
{
	if(!m_buffer.eof())
	{
		IS_FILE_VALID();
...
vfs::size_t vfs::BufferFile::getReadPosition()
{
	IS_FILE_VALID();
	VFS_THROW_IFF(m_isOpen_read || this->openRead(), ERROR_FILE(L"open error"));
...
void vfs::BufferFile::setReadPosition(vfs::size_t positionInBytes)
{
	IS_FILE_VALID();
	VFS_THROW_IFF( m_isOpen_read || this->openRead(), ERROR_FILE(L"open error") );
...
void vfs::BufferFile::setReadPosition(vfs::offset_t offsetInBytes, IBaseFile::ESeekDir seekDir)
{
	IS_FILE_VALID();
	VFS_THROW_IFF( m_isOpen_read || this->openRead(), ERROR_FILE(L"open error") );
...
bool vfs::BufferFile::isOpenWrite()
{
	return m_isOpen_write;
}

bool vfs::BufferFile::openWrite(bool bCreateWhenNotExist, bool bTruncate)
{
	if( !m_buffer.good() )
		return false;
	if( m_isOpen_write )
...
vfs::size_t vfs::BufferFile::write(const vfs::Byte* data, vfs::size_t bytesToWrite)
{
	IS_FILE_VALID();
...
vfs::size_t vfs::BufferFile::getWritePosition()
{
	IS_FILE_VALID();
	VFS_THROW_IFF( m_isOpen_write || this->openWrite(), ERROR_FILE(L"open error") );
...
void vfs::BufferFile::setWritePosition(vfs::size_t positionInBytes)
{
	IS_FILE_VALID();
	VFS_THROW_IFF( m_isOpen_write || this->openWrite(), ERROR_FILE(L"open error") );
...
void vfs::BufferFile::setWritePosition(vfs::offset_t offsetInBytes, IBaseFile::ESeekDir seekDir)
{
	IS_FILE_VALID();
	VFS_THROW_IFF( m_isOpen_write || this->openWrite(), ERROR_FILE(L"open error") );
...
void vfs::BufferFile::copyToBuffer(vfs::ReadableFile_t* file)
{
	try
	{
		bool needToClose = !file->isOpenRead();
		OpenReadFile readfile(file);
		if(!needToClose)
		{
			readfile.release();
		}
...
bool vfs::BufferFile::deleteFile()
{
	m_buffer.clear();
	m_buffer.str("");
