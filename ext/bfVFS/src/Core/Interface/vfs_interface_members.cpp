...
vfs::Path const& vfs::IBaseFile::getName() const
{
	return m_filename;
};

void vfs::IBaseFile::getName(vfs::Path& name) const
{
	name = m_filename;
};

vfs::Path vfs::IBaseFile::getPath() const
{
	return this->getName();
};

void vfs::IBaseFile::getPath(vfs::Path& dir, vfs::Path& file) const
{
	this->getName(file);
}
...
vfs::ILibrary::ILibrary(vfs::ReadableFile_t *libraryFile, vfs::Path const& mountPoint)
: BaseClass_t(mountPoint), m_lib_file(libraryFile)
{
}
