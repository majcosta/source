...
typedef struct{ vfs::String::size_t start, end; } env_t;

bool vfs::Path::expandEnv()
{
	String::ptr_t     raw_ptr     = &_path[0];
	std::stack<env_t> pos;
	vfs::Int32        pos_start   = -1,
	                  pos_end     = -1,
	                  pos_current =  0;
	while(*raw_ptr != 0)
	{
	...
	if(!pos.empty())
	{
		...
		{
			env_t  v        = pos.top();
			String var_name = _path.substr(v.start+2, v.end-v.start-2);
			String var_value;
...
static void unifySeparators(vfs::String::str_t &path)
{
	vfs::String::char_t& raw     = path[0];
...
static void getFirstLastSeparator(const vfs::String::char_t* path, vfs::size_t &iFirst, vfs::size_t &iLast)
{
	iFirst                          = vfs::npos;
	iLast                           = vfs::npos;
	const vfs::String::char_t*  ptr = path;
	vfs::size_t                 pos = 0;
	while(*ptr != 0)
	{
...
{
	return String::less(s1._path.c_str(), s2._path.c_str());
}
bool vfs::Path::Equal::operator ()(vfs::Path const& s1, vfs::Path const& s2) const
{
	return String::equal(s1._path.c_str(), s2._path.c_str());
}
//////////////////////////////////////////////////////////////////////
...
vfs::Path::Path(vfs::String const& path)
: _path(path.c_wcs())
{
	doCheck();
}

vfs::Path::Path(const char* path)
{
	if(Settings::getUseUnicode())
	{
		String::as_utfW(path,_path);
	}
	else
	{
		String::widen(std::string(path),_path);
	}
	doCheck();
}

vfs::Path::Path(std::string const& path)
{
	if(Settings::getUseUnicode())
	{
		String::as_utfW(path,_path);
	...
	{
		String::widen(path,_path);
	}
	doCheck();
}

vfs::Path::Path(const wchar_t* path)
: _path(path)
{
	doCheck();
}
...
std::string vfs::Path::to_string() const
{
	if(Settings::getUseUnicode())
	{
		return String::as_utf8(_path);
	}
	else
	{
		std::string s;
		String::narrow(_path,s);
		return s;
	}
}
...
void vfs::Path::splitLast(vfs::Path& head, vfs::Path& last) const
{

	pathSplitLast        (_path,        head._path,      last._path, _sep.first, _sep.last);
	getFirstLastSeparator(head.c_str(), head._sep.first, head._sep.last);
	getFirstLastSeparator(last.c_str(), last._sep.first, last._sep.last);
...
void vfs::Path::splitFirst(vfs::Path& first, vfs::Path& tail) const
{
	pathSplitFirst       (_path,         first._path,      tail._path, _sep.first, _sep.last);
	getFirstLastSeparator(first.c_str(), first._sep.first, first._sep.last);
	getFirstLastSeparator(tail.c_str(),  tail._sep.first,  tail._sep.last);
	// no need to check, as the original path is already checked
	//rPath.doCheck();
	//rFile.doCheck();
}

bool vfs::Path::extension(vfs::String &ext) const
{
	if(_path.empty())
	{
		return false;
	}
	String::size_t SIZE = _path.length();
	if(_path.at(SIZE-1) == L'.')
	{
		// not an extension
		return false;
	}
	for(String::size_t i=SIZE-2; i > 0; i--)
	{
		if(_path.at(i) == L'.')
		{
			ext.r_wcs().assign(&_path.at(i+1),SIZE-i-1);
			return true;
		}
	}
	return false;
