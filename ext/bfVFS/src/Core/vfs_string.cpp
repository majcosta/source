...
////////////////////////////////////////////////////////////////////
///	                  UTF8/16/32 conversion helper
////////////////////////////////////////////////////////////////////

//! \brief convert UTF8 string to a wchar_t base string depending on size of type wchar_t
template<int WCHAR_LEN>
struct ConvertUTF
{
	template<typename IterStart, typename IterEnd, typename IterOut>
	static void from_utf8(IterStart start, IterEnd end, IterOut out);

	template<typename IterStart, typename IterEnd, typename IterOut>
	static void to_utf8  (IterStart start, IterEnd end, IterOut out);
};


//! \brief conversion with sizeof(wchar_t) == 2
template<>
struct ConvertUTF<2>
{
	template<typename IterStart, typename IterEnd, typename IterOut>
	static void from_utf8(IterStart start, IterEnd end, IterOut out)
	{
		utf8::utf8to16(start, end, out);
	}

	template<typename IterStart, typename IterEnd, typename IterOut>
	static void to_utf8  (IterStart start, IterEnd end, IterOut out)
	{
		utf8::utf16to8(start, end, out);
	}
};

//! \brief conversion with sizeof(wchar_t) == 4
template<>
struct ConvertUTF<4>
{
	template<typename IterStart, typename IterEnd, typename IterOut>
	static void from_utf8(IterStart start, IterEnd end, IterOut out)
	{
		utf8::utf8to32(start, end, out);
	}

	template<typename IterStart, typename IterEnd, typename IterOut>
	static void to_utf8  (IterStart start, IterEnd end, IterOut out)
	{
		utf8::utf32to8(start, end, out);
	}
};
...
vfs::String::str_t vfs::String::as_utfW(std::string const& str)
{
	String::str_t s;
	String::as_utfW(str, s);
	return s;
}
void vfs::String::as_utfW(std::string const& str, vfs::String::str_t &strW)
{
	try
	{
		...
		if(d > 0)
		{
			...
			ConvertUTF<sizeof(wchar_t)>::from_utf8(str.begin(), str.end(), &strW[0]);
		}
	}
	...
	{
		std::wstring err;
		VFS_IGNOREEXCEPTION(String::as_utfW(ex.what(),err), true);
...
vfs::String::str_t vfs::String::as_utfW(const char* str)
{
	str_t s;
	as_utfW(str, s);
	return s;
}
void vfs::String::as_utfW(const char* str, vfs::String::str_t &strW)
{
	if(str == NULL)
	{
		return;
	...
	try
	{
		...
		{
			strW.resize(d);
			ConvertUTF<sizeof(wchar_t)>::from_utf8(str, str+len, &strW[0]);
		}
	}
	catch(utf8::invalid_utf8& ex)
	{
...
std::string vfs::String::as_utf8(std::wstring const& str)
{
	...
	while(pos_in < str.length())
	{
		...
		// convert utf32 character to UTF8 sequence
...
std::string vfs::String::as_utf8(const wchar_t* str, vfs::String::size_t strlength)
{
	...
	if(str != NULL)
	{
		...
		if(len != 0)
		{
			ConvertUTF<sizeof(wchar_t)>::to_utf8(str,str+len,std::back_inserter(s));
		}
	}
	return s;
}
...
{
	String::size_t len = narrow(src.c_str(),src.length(),NULL,0);
	if(len > 0)
	{
		dst.resize(len);
		return narrow(src.c_str(),src.length(),&dst[0],len);
	}
	return 0;
}
...
{
	std::string s;
	String::size_t len = narrow(str,length,NULL,0);
	if(len > 0)
	{
		s.resize(len);
		narrow(str,length,&s[0],len);
	}
	return s;
}
...
{
	std::wstring ws;
	String::size_t len = widen(str,length,NULL,0);
	if(len > 0)
	{
		ws.resize(len);
		widen(str,length,&ws[0],len);
	}
	return ws;
}
vfs::String::size_t vfs::String::widen(std::string const& src, std::wstring& dst)
{
	String::size_t len = widen(src.c_str(),src.length(),NULL,0);
	if(len > 0)
	{
		dst.resize(len);
		return widen(src.c_str(),src.length(),&dst[0],len);
	}
	return 0;
}
...
{
	this->add(vfs::String::as_utfW(value));
	return *this;
}
template<>
