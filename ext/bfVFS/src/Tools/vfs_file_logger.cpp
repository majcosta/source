...
vfs::FileLogger::FileLogger(vfs::WritableFile_t* file, bool append, vfs::Log::EFlushMode flush_mode)
...
{
	*m_log << "[" << m_clock.running() << "]  " << String::as_utf8(msg) << Log::endl;
}

void vfs::FileLogger::Msg(const char* msg)
{
	*m_log << "[" << m_clock.running() << "]  " << msg << Log::endl;
}
