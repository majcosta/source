...
vfs::ReadLine::ReadLine(vfs::ReadableFile_t* file)
: _file(file), _buffer_pos(0), _eof(false)
{
	memset(_buffer,0,sizeof(_buffer));

	OpenReadFile  rfile(_file);
...
vfs::ReadLine::~ReadLine()
{
	if(!_file.isNull() && _file->isOpenRead())
	{
		_file->close();
	}
}

bool vfs::ReadLine::fillBuffer()
{
	if(_eof)
	{
		return false;
...
bool vfs::ReadLine::fromBuffer(std::string& line)
{
	bool done = false;
	while(!done)
	{
		if(_buffer_pos < _buffer_last)
		{
			...
			// need to append substring, as we might have hit the end of the buffer in the previous iteration,
			// then refilled the buffer and now are continuing to extract the line (because there was no \n or \r\n terminator)
			line.append( (char*)&_buffer[start_pos], _buffer_pos - start_pos );

			// if we reach the (real) end of the buffer (that is always terminated with 0), this means
...
bool vfs::ReadLine::getLine(std::string& line)
{
	line.clear();
...
vfs::Tokenizer::Tokenizer(vfs::String const& str)
: m_list(str), m_current(0), m_next(0)
{};

vfs::Tokenizer::~Tokenizer()
{};

bool vfs::Tokenizer::next(vfs::String& token, vfs::String::char_t delimeter)
{
	if(m_next != String::str_t::npos)
	{
 ...
 */
bool vfs::matchPattern(vfs::String const& pattern, vfs::String const& str)
{
	return matchPattern(pattern, str.c_wcs());
}

bool vfs::matchPattern(vfs::String const& pattern, vfs::String::str_t const& str)
{
	String::str_t const& pat  = pattern.c_wcs();
	String::size_t       star = pat.find_first_of(vfs::Const::STAR());

	if(star == String::str_t::npos)
	{
		return StrCmp::Equal( pat, str );
	}
	else if(star == 0)
	{
		if(pat.length() == 1)
		...
		String::char_t atpos1 = pat.at(1);
		String::size_t match  = String::size_t(-1);
		do
		{
			match = str.find_first_of(atpos1,match+1);
			if(match == String::str_t::npos)
			{
				return false;
			}
		} while(!matchPattern( pat.substr(1,pat.length()-1), str.substr(match,str.length()-match) ));
		return true;
	}
	else // if(star > 0)
	{
		// check if characters before * match
		if(!StrCmp::Equal(pat.substr(0,star), str.substr(0,star)) )
		{
			return false;
		}
		return matchPattern( pat.substr(star,pat.length()-star), str.substr(star,str.length()-star) );
	}
}
